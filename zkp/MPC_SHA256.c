...
int do_prove(int argc,char *argv[]) {
	setbuf(stdout, NULL);
	...
	//fgets(userInput, sizeof(userInput), argv[1]);
...
napi_value prove(napi_env env, napi_callback_info info) {
	napi_status status;
	napi_value res;
	//status = napi_create_string_utf8(env, do_verifyRes(), NAPI_AUTO_LENGTH, &res);
	//assert(status == napi_ok);
	NAPI_CALL(env, napi_create_string_utf8(env, do_prove(input), 64, &res));
	return res;
}
